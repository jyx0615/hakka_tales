...
function ImageGrid() {
    ...
    <>
        {Object.keys(books).map((type, typeIndex) => (
            <div key={typeIndex}>
                <div className='fs-2 fw-bolder my-4 ps-4 text-decoration-underline'>{type}</div>
                <Row className='pt-3'>
                {books[type].map((bookName, index) => (
                    <Col key={index} md={4} lg={3} sm={6} xs={12} className="mb-4 bookGrid d-flex flex-column align-items-center px-5"
                        ...
                        <div className="d-flex justify-content-center book-cover">
                            <img src="https://cdn.cite.com.tw/images/g/GD0382C.jpg" alt="placeholder" className='w-100'/>
                        </div>
